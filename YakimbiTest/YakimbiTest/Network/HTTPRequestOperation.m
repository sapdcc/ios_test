...
- (id)initWithRequestURL:(id)requestURL
{
    ...
    if (self)
    {
        if ([requestURL isKindOfClass:[NSURL class]])
        {
            ...
            {
                _requestURL = [[NSURL URLWithString:requestURL] retain];
            }
    }
    return self;
}
...
{
    return [[[HTTPRequestOperation alloc] initWithRequestURL:requestURL] autorelease];
}
...
- (void)main
{
    SAFE_RELEASE(_urlConnection);
    _urlConnection = [[NSURLConnection connectionWithRequest:[NSURLRequest requestWithURL:_requestURL] delegate:self] retain];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    {
        id object = [[_parserClass new] autorelease];
...
{
    return _data ? [[[NSString alloc] initWithData:_data encoding:NSUTF8StringEncoding] autorelease] : nil;
}
