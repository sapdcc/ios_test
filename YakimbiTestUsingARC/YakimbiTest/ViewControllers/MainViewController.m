...
- (void)viewDidLoad
{
    ...
    _fetchOperation = [HTTPRequestOperation requestWithURL:SAMPLE_JSON_URL andDelegate:self];
...
- (void)refreshData
{
    ...
    if ([_users count] > 0)
    {
        ...
        if (_user)
        {
            ...
            _items = [[[_user folders] allObjects] sortedArrayUsingDescriptors:@[asc]];
        }
...
{
    InfoViewController *_infoViewController = [InfoViewController new];
    [_infoViewController setUser:_user];
    
    if (DEVICE_iPAD)
    {
        if (!_popOverViewController)
            _popOverViewController = [[UIPopoverController alloc] initWithContentViewController:_infoViewController];
        
        [_popOverViewController setDelegate:self];
        
        [_popOverViewController presentPopoverFromBarButtonItem:sender permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
    }
    else
    {
