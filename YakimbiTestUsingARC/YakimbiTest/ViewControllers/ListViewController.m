...
- (void)refreshData
{
    ...
    _items = [[[[_folder files] allObjects] subarrayWithRange:range] sortedArrayUsingDescriptors:@[asc]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (DEVICE_iPAD)
    {
        ...
        if (!_popOverViewController)
            _popOverViewController = [[UIPopoverController alloc] initWithContentViewController:_mainViewController];
        [_mainViewController setPopOverViewController:_popOverViewController];
        [_popOverViewController setDelegate:self];

        // Calculate the location to show the UIPopoverController
        CGRect rect = [tableView rectForRowAtIndexPath:indexPath];
        [_popOverViewController presentPopoverFromRect:CGRectMake(rect.origin.x + rect.size.width-25, CGRectGetMidY(rect)+ (int)(rect.size.height/2) - 18 - tableView.contentOffset.y, 1, 1) inView:[self view] permittedArrowDirections:UIPopoverArrowDirectionRight animated:YES];
    }
    else
    {
        UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"Test" delegate:nil cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Row1", @"Row2", @"Row3", @"Row4", nil];
